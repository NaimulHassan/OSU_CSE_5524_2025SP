...
def find_background(args, I):
    ...
    Map_background = (I == 1).astype(float)
...
def find_edge_type(args, I, Map_background, Map_edge, Map_surface):
    ...
    Map_horizontal_edge = (Map_edge == 1) & (edge_angle > np.pi / 4) # If the edge angle is > 45° (i.e., mostly horizontal change), it's a horizontal edge.
    Map_vertical_edge = (Map_edge == 1) & (edge_angle <= np.pi / 4) # If the edge angle is ≤ 45° (i.e., mostly vertical change), it's a vertical edge.
    ...
    rows, cols = Map_contact_edge.shape

    for j in range(cols-1, 0, -1):  # Contact edges will be defined as horizontal edges that transition from foreground (figure) to background (top-down).
        for i in range(rows):
            if Map_horizontal_edge[i, j] == 1:
                flag=0
                # Ensure that every pixel below is background
                for k in range(j-1,-1,-1):
                    if Map_background[i,k]==0:
                        flag=flag+1
                if flag==0:
...
def Y_3D(I, Map_background, Map_horizontal_edge, Map_vertical_edge, Map_contact_edge, Map_surface, theta):
    ...
    for i in range(M):
        for j in range(N):
            ...
            if Map_vertical_edge[i, j] == 1 and j > 0:
                row = np.zeros(M * N)
                row[idx] = 1
                row[i * N + (j-1)] = -1
                A.append(row)
                b.append(1 / np.cos(theta))
            
            if Map_horizontal_edge[i, j] == 1 and i > 0 and j > 0:
                row = np.zeros(M * N)
                row[idx] = (-ny[i, j] + nx[i, j])
                row[i * N + j - 1] = -nx[i, j]
                row[(i-1) * N + j] = ny[i, j]
                A.append(row)
                b.append(0)
            
            if Map_surface[i, j] == 1 and i > 0 and j > 0:
                row1 = np.zeros(M * N)
                row2 = np.zeros(M * N)
                row3 = np.zeros(M * N)
...
def Z_3D(I, yy, Y, theta):
    ...
    Z = (yy - np.cos(theta) * Y) / (-np.sin(theta))
